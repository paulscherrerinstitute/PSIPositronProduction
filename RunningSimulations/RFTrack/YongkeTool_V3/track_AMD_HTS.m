  ...
  target_position = +30; % mm, target exit distance to peak field (30 is the optimised result that can be used for all cases)

  sol_const = 0.5; % T, constant solenoid field

  disp(['INFO:: Target exit position: ' num2str(target_position) ' mm']);
  disp(['INFO:: Constant solenoid field: ' num2str(sol_const) ' T']);
  ...
  amd.Ri  = 15; % Effective inner aperture radius in mm (15 is minimum radius for radiation shielding for all cases)

  amd.half_length = 96.5; % half length of HTS solenoid in mm

  amd.Ztc = target_position; % Target exit location w.r.t HTS upstream solenoid exit in mm
  amd.Bc  = sol_const; % Constant solenoid field for acceleration in T

  %%

  rf.R1i  = 20; % mm, RF iris radius
  
  % Input
  
  amd_input_file = 'Dat/TargetOutputPositrons_E6GeV_SpotSize0.5mm_EmittXY15um_ConvTarget5X0.dat';
  ...
  B_target_6d = Bunch6d(Bunch.mass, 0, Bunch.charge, A_target);
  ...
  outfname = amd_input_file;
  outfname = strrep(outfname,'TargetOutput','AMDOutput');
  outfname = strrep(outfname,'.dat',sprintf("_HTS_%.1fT.dat",amd.Bc));
