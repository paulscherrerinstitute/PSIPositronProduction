  ...
  Bunch.population = 4.37E+10; %% number of particles per bunch (not used)
  ...
  rf.Bc = amd.Bc; % constant field in T
  rf.Ri = 20; % inner aperture radius in mm

  rf.n_cav  = 13; % Nb of structures or cavities [Preliminary]
  rf.Phs = [ 220 220 215 215 210 210 220 270 285 270 240 235 235 ]; % [Preliminary]

  rf.l_cav = 1.2e3; % structure full length in mm
  rf.Grd = [ 18 18 * ones(1,rf.n_cav-1) ]; % dec. acc. ave. gradients in MV/m [Preliminary]
  rf.fgap = 0; % front gap, distance to where 0.5 T starts in mm
  rf.cgap = 150; % cavity gap, distance between cavities in mm [Preliminary]
  ...
  outfname = strrep(outfname,'.dat','_PSISband.dat');
